// src/request_activity/application/SendRequestActivity.ts
...
export class SendRequestActivity {
  ...
  async execute(requestData: RequestActivity, videoBase64?: string) {
    try {
      ...
      // 2. Check for existing activity request
      ...
      const activityObjectId = new mongoose.Types.ObjectId(requestData.activityId);
      ...
      const [requester, recipient] = await Promise.all([
        UserModel.findOne({ userId: requestData.reqFrom }),
        UserModel.findOne({ userId: requestData.reqTo })
      ]);
      ...
      const errorMessage = error.message.includes("already") ? 
        error.message : 
        (error.code === 11000 ? "Duplicate request detected" : "Failed to send request");
