...
export class ChatController implements IController {
    ...
    private initializeRoutes() {
        this.router.post(`${this.path}/messages`, authenticate, this.sendMessage.bind(this));
        this.router.get(`${this.path}/conversations`, authenticate, this.getConversations.bind(this));
        this.router.get(`${this.path}/messages/:recipientId`, authenticate, this.getMessages.bind(this));
        this.router.put(`${this.path}/messages/read/:recipientId`, authenticate, this.markAsRead.bind(this));
        this.router.get(`${this.path}/chat-lists`, authenticate, this.getChatList.bind(this));
    }
    ...
    private async sendMessage(req: Request, res: Response) {
        try {
            ...
            const message = await this.chatService.sendMessage(senderId, recipientId, content);
